...
### 2.5 API + UI
- **Axum** serving (as implemented through M4, extended at M5): `/api/health`, `/api/config` (globe arc origin + `needs_setup` first-run flag + `version` — see below), `/api/stats`, `/api/devices` (+ `/rename`, `/merge`), `/api/devices/:id/scorecard`, `/api/snapshots`, `/api/sources` (M5 — `GET` lists configured sources *without secrets*; `POST` probes-then-persists-then-starts a source at runtime), `/api/sources/test` (M5 — `POST` validates a URL+token without persisting), `/api/arcs?window=<hours>` (device→country arc aggregates; destinations without a mapped country are excluded but disclosed as `unmapped_queries`), `/api/arcs/domains?device=&country=` (click-through level 1), `/api/rollups?device=&domain=` (level 2 — the raw hourly buckets, the rawest data retained per D-005), `/api/diffs` (M5 — per-device week-over-week change across the two most recent snapshot weeks; new-this-week domains computed from `query_rollups` week windows since snapshots store only counts), and `/api/stream` (SSE). Endpoints are documented here and in CLAUDE.md rather than via OpenAPI (an OpenAPI spec is a possible follow-up, not an M1 artifact as originally planned).
- **Config surface (M5 setup wizard):** through M4 the only config was env (`PHONEHOME_PIHOLE_*`/`_ADGUARD_*`/`_FIXTURE`, `_HOME_LAT/LON`), read once at startup. M5 adds a persistence + runtime-reconfig path so a stranger can install without editing env: the wizard `POST /api/sources` probes the source (reusing the adapter auth), writes it to the `source_config` table (D-014 — secret plaintext, local-only, never returned), sets the optional home in `meta`, and **starts a new ingest loop at runtime** (no restart) via a `spawn_source` registry of `JoinHandle`s in `AppState`. At boot, env sources *and* persisted sources both go through `spawn_source`, so the two coexist. `/api/config` is now computed per-request: `home` = persisted (wizard) home else env home; `needs_setup` = true only when no source is configured *any* way (env or persisted), keyed on source existence rather than data volume so a valid-but-quiet source never re-triggers the wizard. A `PHONEHOME_FIXTURE` deployment (dev/CI/demo) counts as a startup source → wizard hidden.
...
- **Globe (implemented at M4, `ui/src/globe/`):** three.js 0.185 `WebGPURenderer` with automatic WebGL2 fallback (`?gl=1` forces it for A/B runs). All arcs are ONE instanced draw: a 64-segment ribbon bent along the great circle in a TSL vertex stage, per-instance `InstancedBufferAttribute`s only so the same node graph compiles to WGSL and GLSL (D-013). Arcs are rate-aggregated per (canonical device × destination country) from `/api/arcs`, colored by tracker share (emerald→amber→rose), originate at the configured home (`/api/config`), and carry a traveling comet head; SSE pulses transiently boost brightness and trigger a debounced refetch when a new (device, country) pair appears. Device filter rail writes a per-instance visibility flag (no rebuild); click picking projects 9-sample CPU polylines (stress arcs are non-pickable); click-through: arc → domain list → raw hourly rollups (2 clicks). Country centroids + labels come from a bundled generated table (`countryCentroids.ts`); `?stress=N` renders N synthetic arcs for the perf gate, `?hud=1` overlays live frame stats, `?hero=1` runs the GIF choreography, and `window.__phonehome` exposes the Playwright test contract. Perf target: smooth at ≥10k visible arcs on integrated graphics (measured at M4, PROOF §M4).

## 3. Storage sketch (SQLite, WAL mode)

```sql
-- implemented at M1–M3, extended at M5 (schema_version 4):
sources(id PRIMARY KEY, kind, cursor, last_ok_at)          -- hot-path runtime state (cursor); rewritten every batch
source_config(id PRIMARY KEY, kind, base_url, username,    -- M5 wizard: persisted connection config, secret plaintext
              secret, interval_s, enabled, created_at)     --   local-only (D-014), NEVER returned by any API
meta(key PRIMARY KEY, value)                               -- KV: schema_version, + M5 home_lat/home_lon
